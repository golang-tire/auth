...
func (r repository) Get(ctx context.Context, uuid string) (entity.Domain, error) {
	...
	err := r.db.With(ctx).Model(&domain).Where("uuid = ?", uuid).First()
	return domain, err
}
...
func (r repository) Create(ctx context.Context, domain entity.Domain) (string, error) {
	...
	_, err := r.db.With(ctx).Model(&domain).Insert()
...
func (r repository) Count(ctx context.Context) (int64, error) {
	var count int
	count, err := r.db.With(ctx).Model((*entity.Domain)(nil)).Count()
...
func (r repository) Query(ctx context.Context, offset, limit int64) ([]entity.Domain, int, error) {
	...
	count, err := r.db.With(ctx).Model(&_domains).
		Order("id ASC").Limit(int(limit)).
		Offset(int(offset)).SelectAndCount()
	return _domains, count, err
}
