...
const Objects = props => {
    ...
    const columns = [
        {
            ...
            render: (text, record) => <Link to={`${ITEMS}/edit/${record.uuid}`}>{text}</Link>,
        },
        {
            title: 'App',
            dataIndex: 'app',
            render: (text, record) => <Link to={`apps/edit/${record.app.uuid}`}>{record.app.name}</Link>,
        },
        {
            title: 'Created at',
    ...
    const deleteItem = (record) => {
        confirm({
            ...
            content: "Are you sure you want to delete `" + record.identifier + "` ?",
            onOk() {
                ApiService.delete(URL, record.uuid).then(
                    (result) => {
                        message.info("`" + record.identifier + "` removed")
